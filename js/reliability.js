...
$( document ).ready(function() {
	DoXMLHttpRequest('/storage/c-coi.json', parseCCOI);
	
	$.each(sessionsData, function( index, value ) {
		sessions[index] = new CCOI_Session(value);
	});
...
function interraterTimeCheck (session1, session2) {
  ...
  // This sets how many paths the coders are allowed to differ on. i.e., 1 means that if coder 1 has
  // 3 paths, then coder 2 may have somewhere between 2 and 4 paths
  const NUM_PATH_DIFFERENCE_TOLERANCE = 0;
  // This is the number of seconds that events from two different coders are allowed to differ in their start
  const EVENT_TIME_TOLERANCE_SECS = 5;
  if (Math.abs(session1.paths.length - session2.paths.length) > NUM_PATH_DIFFERENCE_TOLERANCE) {
    return [`Session "${session1.name}" has ${session1.paths.length} paths, ` +
      `Session "${session2.name}" has ${session2.paths.length} paths`];
  } else {
	var reasonsToFail = [];
	var isFailing = false;
    const numPaths = session1.paths.length < session2.paths.length ? session1.paths.length : session2.paths.length;
    for (let pathIdx = 0; pathIdx < numPaths; pathIdx++) {
      const path1 = session1.paths[pathIdx].steps;
      const path2 = session2.paths[pathIdx].steps;
      ...
      if (Math.abs(path1[0].timeInSeconds - path2[0].timeInSeconds) > EVENT_TIME_TOLERANCE_SECS){
		isFailing = true;
		var path1Start = path1.steps[0].timeInSeconds;
		var path2Start = path2.steps[0].timeInSeconds;
        reasonsToFail.push(`Session "${session1.name}", path ${pathIdx+1} begins at ${path1Start} seconds, `+
			`Session "${session2.name}", path ${pathIdx+1} begins at ${path2Start} seconds. `);
	  }
	  if(Math.abs(path1[path1.length-1].timeInSeconds - path2[path2.length-1].timeInSeconds) > EVENT_TIME_TOLERANCE_SECS) {
		isFailing = true;
		var path1End = path1[path1.length-1].timeInSeconds;
		var path2End = path2[path2.length-1].timeInSeconds;
        reasonsToFail.push(`Session "${session1.name}", path ${pathIdx+1} ends at ${path1End} seconds, `+
			`Session "${session2.name}", path ${pathIdx+1} ends at ${path2End} seconds. `);
      }
    }

	if(isFailing)
		return reasonsToFail;
...
function interRaterReliabilityCheck(session1, session2) {
	...
	if(interraterTimeCheck(session1, session2) !== true){
		return "IRR level 1 failed; not running IRR2.";
	}
	...
	// We can assume that the sessions have the same number of paths if they passed IRR1.
	...
	for(var i = 0; i < numPaths; i++){
		...
		const path2 = session2Copy.paths[i].steps;
		console.log("path1", path1, "path2", path2)
		// Create empty agreement array
		var agreementPath = [0,0];
		...
		for(var j = 0; j < path1.length; j++) {
			...
			for(var k = 0; k < path2.length; k++) {
				...
				if(!foundAgreement &&
				   ...
				   stepToMatch.choice == stepToCheck.choice &&
	...
	// Decide percent agreement for the path
	var n = 0;
	var d = 0;
