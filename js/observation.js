...
var ccoiObservation = (function (){
    ...
    function addBranch (value, oldValue, description, checked, divToAttachTo, aside = false) {
        ...
        branchForm.append(`
            <p>
                <input type="radio" name="choiceRadio" id="choiceRadio${value}" data-oldChoiceIndex="${oldValue}" value="${value}" ${checked==true ? 'checked' : ''}>
...
//        console.log("this is the current session number in setupNode Branches: " + currentSessionID)
  //      console.log("SetUpNodeBranches: CurrentSession is: " + sessions[currentSessionID]);
        ...
        if (node.should_group_choices) {
            ...
            for (let groupIndex = 0; groupIndex < numGroups; groupIndex++) {
                ...
                for (let branchIndex = 0; branchIndex < numBranches; branchIndex++) {
                    ...
                    let isSelected = selectedChoice == branch.branch_new_id || selectedChoice === branch.pretty_id;
                    addBranch(branch.branch_new_id, branch.branch_id, '(' + ChoiceID(branch.pretty_id) + ') ' + branch.description, isSelected, `#collapse_${groupIndex}_body`, branch.aside);
                    radioButtonIndex++;
                }
            }
            ...
            for(let i=0; i < branches.length; i++) {
                for (let j = 0; j < branches[i].length; j++) {
                    ...
                    let isSelected = selectedChoice == branch.branch_new_id || selectedChoice === branch.pretty_id;
                    addBranch(branch.branch_new_id, branch.branch_id, '(' + ChoiceID(branch.pretty_id) + ') ' + branch.description, isSelected, '#branch_radio_form', branch.aside);
                }
    ...
    function deletePath(deleteIndex) {
        if (confirm('Are you sure you want to delete path #' + (deleteIndex + 1) + '?')) {
            ...
            if(alteredSessionData.paths[deleteIndex] == undefined) alteredSessionData.paths[deleteIndex] = pathsCopy[deleteIndex];
    ...
    function getSessions() {
        if(isDemo) {
            ...
            if (jsUserVars['pid'] != undefined) {
                ...
                if (conf == true) { window.location.replace("/login"); } else { window.location.replace("/login"); }
            }
        }
    }
