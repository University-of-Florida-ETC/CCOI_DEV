(function (global, factory) {
  ...
  // return the value of a node or link
  function incomingValue(d) {
    return d3Array.sum(d.targetLinks, value);
  }                                                                     
  ...
  function sankeyCircular () {
    ...
    function computeNodeValues(graph) {
      graph.nodes.forEach(function (node) {
        ...
        //MCB Added - For graphical purposes, we want minimum link height to be 10 so node values have to be adjusted accordingly
        let sourceValue = 0; 
        let targetValue = 0;
          
        for (let i=0; i<node.sourceLinks.length; i++) {
            sourceValue += (node.sourceLinks[i].value + 10);
        }
        for (let j=0; j<node.targetLinks.length; j++) {
            targetValue += (node.targetLinks[j].value + 10);
        }
        let newValue = Math.max(sourceValue, targetValue);
