...
$query = "SELECT s.id, s.videoid FROM tbSessions s, tbPeopleAppSessions pas WHERE pas.appid='1' AND pas.sessionid=s.id AND videoid IS NOT NULL AND s.inactive IS NULL AND pas.inactive IS NULL";
$return = mysqli_query($db, $query);
while ($p = mysqli_fetch_assoc($return)) {
	$sesscount[$p['videoid']]++;
}			//print_r($sesscount);

$query = "SELECT id, name FROM tbVideos WHERE appid='1' AND inactive IS NULL";
$return = mysqli_query($db, $query);
while ($p = mysqli_fetch_assoc($return)) {
	$vidlist .= "<option value='{$p['id']}'>{$p['name']} ({$sesscount[$p['id']]})</option>";
}
echo "<select id='vidList'><option disabled selected>Select a video</option>{$vidlist}</select>";
?>

<select id='sessionA'>
	<option disabled selected>Select a session to compare</option>
</select>
<select id='sessionB'>
	<option disabled selected>Select a session to compare</option>
</select>
...
<script language='javascript'>
	...
	if (vidList) {
		vidList.addEventListener('change', function(e) {
			getSessions(e);
			e.stopPropagation();
		}, false);
	}
