...
//				echo '<p>your token is good</p>';
	...
	if($CCOI_requireslogin && empty($_SESSION['pid'])){
		...
		header('Location: https://ccoi-dev.education.ufl.edu/login.php');
	}
	
	if($CCOI_requiresadmin && empty($_SESSION['roles']['admin'])){
		$msg['errorType']='Role Failure: Admin';     $output=json_encode($msg);     echo $output;     //exit;
	}

	if($CCOI_requirescoder && empty($_SESSION['roles']['coder'])){
...
// 		$msg['errorType']='Role Failure: Videosync';     $output=json_encode($msg);     echo $output;     //exit;
// 	}
