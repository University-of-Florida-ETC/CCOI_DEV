...
if(!empty($_FILES['derfile'])){
		...
		if(isset($errors)){
			...
			if(move_uploaded_file($_FILES['derfile']['tmp_name'],"$root/$newfilename")){
				...
				$query="INSERT INTO tbVideos (appid,name,url,scramble,viddate,teacher,compno,videxists) VALUES ('1','$dername','$derurl','$scramblename','$derviddate','$derteach','$dercompno','1')";			// additional data can be provided through the video manager
