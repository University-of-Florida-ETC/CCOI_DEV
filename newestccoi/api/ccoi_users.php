...
$includeroot= $_SERVER['DOCUMENT_ROOT'];			$devprodroot='/newestccoi';			$serverroot=$_SERVER['SERVER_NAME'];				// this is PROD -- nothing for dev
...
if(!empty($_POST['yahoo']) && !empty($_POST['email']) && !empty($_POST['newpass'])){
	...
	//	echo $query;
		...
		if(mysqli_affected_rows($db)==1){
			...
			$query="INSERT INTO tbPersonAppRoles (personid,appid,role) VALUES ('$newid','{$_SCRUBBED['appid']}','coder')";			//echo $query;	// user always exists even if the person is de-activated
			$return=mysqli_query($db,$query);
			if(mysqli_affected_rows($db)==1){
