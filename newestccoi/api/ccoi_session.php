...
//				echo '<p>your token is good</p>';
	...
	if($CCOI_requireslogin && empty($_SESSION['pid'])){
		...
		header("Location: https://{$_SERVER['SERVER_NAME']}/newestlogin.php");		// ===== add "newlogin" for prod =======================================
	}
	...
	if($CCOI_requiresadmin && empty($_SESSION['roles'][1]['admin'])){
		$msg['errorType']='Role Failure: Admin';     $output=json_encode($msg);     echo $output;    print_r($_SESSION); //exit;
	}
