...
//		if(!$db){$err=mysqli_error();		exit ("Error: could not connect to the CCOI Database! -- $access -- $err");}
		...
		$return=mysqli_query($db,"SELECT * FROM tbPeople WHERE email='$washedemail' AND inactive IS NULL");		$persondata=mysqli_fetch_assoc($return);

		if( !password_verify($_POST['password'],$persondata['passhash']) ){
			header("Location: https://{$_SERVER['SERVER_NAME']}/newestlogin.php"); session_write_close(); exit;
		}else{
		...
		//		$_SESSION['roles'][$roledata['role']]=true;
				if(!in_array($roledata['appid'], $_SESSION['myappids'])){
					...
					//$_SESSION['myappids2'][] = $roledata['appid'];
				}
	...
	//		$_SESSION['myapps'][1]=true;
			...
			if(!empty($_SESSION['loginroute'])){
				...
				header("Location: https://{$_SERVER['SERVER_NAME']}/newestccoi/");					session_write_close();		exit;
